...
copy_ssh_key() {
    ...
    ssh-copy-id -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -i "$ssh_key.pub" "debian@$vps_ip" > /dev/null 2>&1
    ...
    if [ $? -eq 0 ]; then
        ...
        # Alternative method
        cat "$ssh_key.pub" | ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -i "$ssh_key" "debian@$vps_ip" "mkdir -p ~/.ssh && cat >> ~/.ssh/authorized_keys && chmod 600 ~/.ssh/authorized_keys" > /dev/null 2>&1
...
add_debian_to_sudo() {
    ...
    if id "debian" &>/dev/null; then
        ...
        echo "User 'debian' added to sudo group."
        
        # Also ensure debian user has proper shell
        if ! grep -q "^debian:" /etc/passwd | grep -q "bash"; then
            usermod -s /bin/bash debian
            echo "Shell set to /bin/bash for debian user."
        fi
    else
        echo "Warning: User 'debian' does not exist. Creating debian user..."
        # Create debian user with sudo privileges
        useradd -m -s /bin/bash debian
        echo "debian:$(generate_password 16)" | chpasswd
        usermod -aG sudo debian
        echo "User 'debian' created and added to sudo group."
    fi
}
...
configure_ssh() {
    ...
    # Create new SSH configuration for Debian
    cat > /etc/ssh/sshd_config << EOL
# Generated by automated setup script for Debian
Port $SSH_PORT
Protocol 2
PermitRootLogin yes
...
install_dependencies() {
    echo -e "Installing required dependencies for Debian..."
...
main() {
    echo "=== Automated System Setup for Debian ==="
    echo "This script will:"
    echo "1. Update system packages"
    echo "2. Set auto-generated root password"
    echo "3. Configure sudo access for debian user"
...
# Main script
main() {
    echo "=== COMPLETE VPS SETUP AUTOMATION FOR DEBIAN ==="
    echo ""
    ...
    echo "Menjalankan setup lengkap di VPS Debian..."
